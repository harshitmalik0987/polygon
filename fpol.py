...
# ---- CONFIG ----
TOKEN = '7682816850:AAHk0wNFmZsu5wvMTZl1AzWTCloBHM7s9ZU'
POLYGON_RPC_URL = "https://polygon-rpc.com"
ADMIN_PRIVATE_KEY = "e2bad866afa6ea69af2296e8ae13d40c6e7153997713135ea01842c612591ba5"
ADMIN_PASSWORD = 'h234'
MIN_WITHDRAW = 0.08
REFER_BONUS = 0.02
SIGNUP_BONUS = 0.05
...
def set_wallet(m):
    ...
    bot.send_message(user_id, msg, reply_markup=cancel_markup())
...
def change_wallet(call):
    user_id = call.from_user.id
    bot.send_message(user_id, "✏️ <b>Send your new Polygon Wallet Address</b>\n\n(Existing wallet will be replaced, must be unique)\n\nType '❌ Cancel' to exit.", reply_markup=cancel_markup())
...
def process_wallet(m):
    ...
    text = m.text.strip()
    if text == '❌ Cancel':
        send_main_menu(user_id, "❌ <b>Cancelled. Back to main menu.</b>")
        return
    wallet = text
    if not web3.is_address(wallet):
        bot.send_message(user_id, "❌ <b>Invalid Polygon wallet address. Try again or type '❌ Cancel' to exit.</b>", reply_markup=cancel_markup())
        bot.register_next_step_handler(m, process_wallet)
        return
    ...
    if exist and exist[0] != user_id:
        bot.send_message(user_id, "🚫 <b>This wallet is already used by another user. Try another or type '❌ Cancel' to exit.</b>", reply_markup=cancel_markup())
        bot.register_next_step_handler(m, process_wallet)
        return
...
def process_withdraw(m, bal, wallet):
    ...
    text = m.text.strip()
    if text == '❌ Cancel':
        send_main_menu(user_id, "❌ <b>Withdraw cancelled. Back to main menu.</b>")
        return
    try:
        amt = float(text)
        if amt < MIN_WITHDRAW or amt > bal:
            bot.send_message(user_id, f"❌ <b>Enter valid amount (Min {MIN_WITHDRAW} POL, Max {bal:.2f} POL) or type '❌ Cancel' to exit.</b>", reply_markup=cancel_markup())
...
def send_polygon_payment(to_address, amount):
    ...
    tx = {
        ...
        'value': value,
        'gas': 21000,
        'gasPrice': web3.to_wei('50', 'gwei'),  # Set to 50 gwei
        'nonce': nonce,
        'chainId': 137,
    }
